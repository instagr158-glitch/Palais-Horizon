...
type PlatformKey = "lofty" | "ark7" | "arrived";

const PLATFORMS: {
  key: PlatformKey;
  name: string;
  url: string;
  /** Price of one share in USD, when the platform sells fixed-price shares. */
  sharePriceUsd: number | null;
  /** Whether people living outside the United States can invest. */
  openWorldwide: boolean;
}[] = [
  { key: "lofty", name: "Lofty", url: "https://www.lofty.ai", sharePriceUsd: 50, openWorldwide: true },
  { key: "ark7", name: "Ark7", url: "https://ark7.com", sharePriceUsd: 20, openWorldwide: false },
  { key: "arrived", name: "Arrived", url: "https://arrived.com", sharePriceUsd: null, openWorldwide: false },
];
...
export default async function InvestMiamiPage() {
  ...
  const descriptions = { lofty: t.loftyDesc, ark7: t.ark7Desc, arrived: t.arrivedDesc };
  const minimums = { lofty: t.loftyMin, ark7: t.ark7Min, arrived: t.arrivedMin };
  const eligibility = { lofty: t.loftyEligibility, ark7: t.usOnlyEligibility, arrived: t.usOnlyEligibility };
          ...
          {PLATFORMS.map((p) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {p.openWorldwide ? t.openBadge : t.usBadge}
                </span>
              </div>
              ...
              <dl className="mt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between gap-4 border-t border-ink-border pt-2">
                  ...
                  <dd
                    className={`text-right ${p.openWorldwide ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {eligibility[p.key]}
                  </dd>
                </div>
