export const en = {
  code: "en",
  tagline: "Thailand's finest homes, one private address book.",
  short: "The members-only index of luxury property across Thailand.",

  nav: {
    listings: "Residences",
    track: "Track",
    invest: "Buy shares",
    pricing: "Membership",
    about: "About",
    account: "My account",
    login: "Sign in",
    join: "Become a member",
    signOut: "Sign out",
    language: "Language",
    memberBar: "MEMBER ACCESS ACTIVE",
  },

  track: {
    title: "Track — Find a home that matches you",
    back: "Back",
    typeLabel: "PROPERTY TYPE",
    typeQuestion: "What are you looking for?",
    typeRent: "Rent",
    typeBuy: "Buy",
    destinationLabel: "DESTINATION",
    destinationQuestion: "Where are you looking?",
    budgetLabel: "YOUR BUDGET",
    budgetQuestion: "What's the maximum monthly rent you want to pay?",
    budgetQuestionBuy: "What's the maximum budget you want to spend?",
    budgetHint: "Drag the slider",
    budgetMinHint: "Small budget",
    budgetMaxHint: "Very ambitious",
    continue: "Continue",
    searchingLabel: "SEARCHING",
    searchingQuestion: "We're analysing the listings that match your criteria…",
    searchDoneTitle: "Your search is ready",
    searchDoneBody: "Unlock the collection to discover the listings that match your criteria.",
    viewListingsCta: "See available listings",
  },

  gate: {
    title: "Choose your language",
    subtitle: "You can change it anytime from the menu.",
  },

  landing: {
    heroTitleLead: "The agent that finds ",
    heroTitleHighlight: "the best real estate prices",
    heroTitleTrail: " before anyone else.",
    heroSubtext:
      "Palais Horizon continuously scans every agency's listings to surface the best prices and the newest properties first.",
    heroCtaPrimary: "Get started",
    demoCaption: "The Palais Horizon interface — search, filter and compare every agency's listings in one place.",
    showcaseRentBadge: "Real listings — Rent",
    showcaseRentTitle: "Rent a home",
    showcaseRentSubtext: "A handful of real, currently-listed rentals.",
    showcaseMiamiBadge: "Real listings — Miami",
    miamiHighlightTitle: "Miami - the American dream",
    baliHighlightTitle: "Bali - the island of the gods",
    highlightCta: "See listings",
    showcaseSaleBadge: "Real listings — Buy",
    showcaseSaleTitle: "Buy a home",
    showcaseSaleSubtext: "A handful of real, currently-listed villas for sale.",
    heroCta: "View membership",
    seeAllListings: "See all listings",
    heroCtaSecondary: "How it works",
    statListingsSuffix: "verified luxury listings",
    statAgenciesSuffix: "partner agencies",
    toolsBadge: "Features",
    toolsTitle: "Four tools. One habit before you invest.",
    tools: [
      {
        label: "Search",
        title: "Find the right listing in seconds.",
        body: "Instantly filter hundreds of listings already gathered in one place by region, property type, budget and offer type.",
        points: [
          "Filters by region, budget, property type and offer type",
          "Search by keyword, area or project name",
          "Results sorted by newest or by price",
        ],
      },
      {
        label: "Comparison",
        title: "Compare prices across agencies, in one place.",
        body: "Palais Horizon gathers every agency's listings so you can compare prices at a glance, without opening ten different sites.",
        points: [
          "Prices shown in EUR and USD",
          "Every agency gathered into one catalogue",
          "Spot the best price per square metre instantly",
        ],
      },
      {
        label: "New listings",
        title: "New listings, before anyone else.",
        body: "The catalogue refreshes continuously, so members discover new properties the moment they're listed.",
        points: [
          "Catalogue refreshed continuously",
          "Signature listings highlighted",
          "The luxury segment only",
        ],
      },
      {
        label: "Introduction",
        title: "A direct line to the agency, no middlemen.",
        body: "Every listing links back to the agency that holds the mandate — you deal with them directly, not with us.",
        points: [
          "Direct link to the agency's own listing",
          "No Palais Horizon middleman fee",
          "The mandated agency's details are visible",
        ],
      },
    ],
    coverageBadge: "Global coverage",
    coverageTitle: "Real-estate listings from around the world, analysed.",
    coverageSubtext:
      "Palais Horizon continuously processes listings everywhere the platform operates, to surface the most profitable properties before anyone else.",
    coverageThailand: "Thailand",
    coverageMiami: "Miami",
    coverageDubai: "Dubai",
    coverageBali: "Bali",
    coverageActive: "Active market",
    coverageSoon: "Coming soon",
    coverageNetherlands: "Netherlands",
    coverageUK: "United Kingdom",
    coverageFrance: "France",
    coverageNewYork: "New York",
    coverageLosAngeles: "Los Angeles",
    lockedPrice: "Price reserved for members",
    subscribeCta: "Subscribe",
    unlockBanner:
      "The full collection — prices, locations, galleries and agency introductions — opens with membership.",
    compareBadge: "Price comparison",
    compareTitle: "The right price, before you buy.",
    compareBody:
      "Palais Horizon gathers the same kind of property across every agency, so the price gap between listings becomes obvious instead of hidden in ten separate tabs.",
    compareChartCaption: "Comparable listings, sorted by price — lowest first.",
    compareStatLabel: "Price gap seen on comparable listings",
    compareStatValue: "up to 30%",
    howTitle: "How it works",
    how: [
      { step: "01", title: "Browse 4 markets", body: "Thailand, Bali, Dubai and Miami — hundreds of real listings from every agency, gathered in one place." },
      { step: "02", title: "Compare the prices", body: "Palais Horizon lines up similar listings so the real price gap between agencies is obvious before you commit." },
      { step: "03", title: "Become a member", body: "Pay securely by card — no account needed to get started. Unlock full prices, galleries and a direct introduction to the agency." },
    ],
    faqTitle: "Questions",
    faq: [
      {
        q: "Do you sell the properties yourselves?",
        a: "No. Palais Horizon is an index. Every listing links to the agency that holds the mandate — you deal with them directly.",
      },
      {
        q: "Where do the listings come from?",
        a: "Leading luxury agencies and portals. We normalise their listings into one searchable collection and keep the link back to the source.",
      },
      {
        q: "Can I cancel?",
        a: "Anytime, in one click from your account. You keep access until the end of the paid period.",
      },
    ],
  },

  invest: {
    badge: "Buy shares in Miami",
    title: "Buy shares of rental properties in Miami",
    subtitle: "We gather offers to buy shares of rental properties, or a whole apartment, in Miami. You buy directly from the platform or agency we send you to: we never collect your money.",
    fractionalTitle: "Start small: shares of rental properties",
    fractionalBody: "Fractional platforms let you buy shares of rental properties and receive a share of the rental income.",
    directTitle: "Buy an apartment that allows short-term rentals",
    directBody: "Some Miami buildings allow Airbnb-style stays. Rules differ from one building to the next and change over time, so always confirm before buying.",
    minLabel: "Minimum",
    incomeLabel: "Income",
    ark7Desc: "Own shares of rental properties. Miami properties are listed in the Ark7 app (availability changes).",
    ark7Min: "$20 per share",
    arrivedDesc: "Shares of single-family rentals and vacation rentals in several US states. Check current availability in Florida.",
    arrivedMin: "$100",
    income: "Monthly distributions, not guaranteed",
    visitAt: "Buy shares at",
    ruleDaily: "Daily rentals allowed",
    rule90: "90-day minimum stay, up to 4 times a year",
    buildingsSource: "Building rules as reported by CondoBlackBook on September 25, 2026. They can change: confirm with the building before buying.",
    fullList: "See the full list at CondoBlackBook",
    disclaimerTitle: "Important",
    disclaimer: "Palais Horizon is an index of third-party information. This is not investment advice. Any investment carries a risk of losing part or all of your capital, and past results do not guarantee future ones. We do not collect funds, promise any return or take part in any transaction: check the terms directly with the platform or agency.",
    updated: "Information gathered on September 25, 2026.",
    bannerTitle: "New: buy shares of Miami rental properties",
    bannerBody: "Real opportunities, no subscription. We send you to the platform or agency.",
    bannerCta: "Discover",
    catalogueTitle: "Best buying opportunities, ranked by estimated yield",
    catalogueBody: "Real apartments in central Miami, currently for sale. For each one we estimate the gross rental yield from the asking rents of comparable homes in the same building or ZIP code.",
    topBadge: "Top yield",
    priceLabel: "Price",
    estRentLabel: "Estimated monthly rent",
    yieldLabel: "Estimated gross yield",
    rangeLabel: "Range",
    confidenceLabel: "Reliability of the estimate",
    confidenceHigh: "Good",
    confidenceMedium: "Medium",
    confidenceLow: "Low",
    basisBuilding: "Based on {n} comparable asking rents ({beds}-bedroom) in “{place}”",
    basisZip: "Based on {n} comparable asking rents ({beds}-bedroom) in ZIP code {place}",
    yieldVsMedian: "Catalogue median: {median} % ({diff} pt)",
    pricePerSqm: "Price per m²: {value}",
    pricePerSqmVs: "{pct} % vs catalogue median",
    bedsShort: "bd",
    bathsShort: "ba",
    viewListing: "View the listing at the agency",
    caveat: "Gross estimate, before condo fees, property tax, insurance, vacancy and costs. It relies on asking rents, not realised income, and says nothing about short-term (Airbnb) rentals, whose rules differ from one building to the next: check them before buying.",
    methodTitle: "How we calculate",
    methodBody: "The estimated rent is the median of the asking rents of comparable homes (same number of bedrooms) in the same building or, failing that, the same ZIP code, taken from the listings in our catalogue. Gross yield is the annual rent divided by the purchase price. Only properties with at least 3 comparables are ranked, and results above 12 %, which are unlikely to be realistic, are left out.",
  },

  paywall: {
    expiredTitle: "Your membership has ended",
    expiredBody: "Reactivate to open the full collection again.",
    expiredCta: "Reactivate membership",
  },

  join: {
    title: "Create your account",
    body: "Enter your email below and we'll send you a link to continue on Palais Horizon in your browser.",
  },

  pricing: {
    title: "Membership",
    body: "One membership. The entire luxury collection, and a direct line to every agency.",
    unlockTitle: "Buy / rent: Find exclusive properties and save up to -30%",
    unlockTagline: "Real estate search",
    monthlyLabel: "Monthly",
    annualLabel: "Annual",
    annualNote: "2 months free",
    bestValue: "Best value",
    perMonth: "/ month",
    perYear: "/ year",
    billedMonthly: "Billed monthly. Cancel anytime.",
    billedAnnual: "Billed yearly. Cancel anytime.",
    cancelAnytime: "Cancel anytime.",
    cta: "Continue",
    redirecting: "Redirecting…",
    emailHint: "Enter your email to create your account and continue in your browser.",
    emailPlaceholder: "you@email.com",
    emailCta: "Create an account",
    emailSending: "Creating…",
    emailSent: "We've sent you an email — if you don't see it, check your spam folder.",
    emailInvalid: "Please enter a valid email address.",
    features: [
      "All local listings in one place",
      "The best-priced properties",
      "Exact locations and map",
      "Complete photo galleries",
      "Direct introduction to the listing agency",
      "Unlimited searches and filters",
      "New luxury listings as they land",
    ],
    notConfigured:
      "Payment is not configured yet. Add your Stripe keys to enable checkout.",
    ownerNote:
      "Note for the site owner: add your Stripe keys and price IDs to .env to enable real checkout. Until then, use the seeded account member@palaishorizon.com to preview member access.",
    disclaimer:
      "Palais Horizon is an index of third-party listings. Membership buys access to the collection and a direct introduction to each listing agency — it is not a brokerage fee and no property transaction is handled here.",
    lockedBanner:
      "The residences catalogue is for members. Choose a plan below to unlock it.",
    canceledBanner: "Checkout canceled — no charge was made.",
    signInFirst: "Please sign in first.",
    networkError: "Network error. Please try again.",
    promoLabel: "Limited-time offer",
  },

  auth: {
    signInTitle: "Sign in",
    signInBody: "Access your Palais Horizon membership.",
    emailLabel: "Email",
    passwordLabel: "Password",
    signInCta: "Sign in",
    signingIn: "Signing in…",
    noAccount: "No account?",
    createOne: "Create one",
    badCredentials: "Email or password is incorrect.",
    registerTitle: "Create your account",
    registerBody:
      "Registration is free. Choose a membership afterwards to unlock the collection.",
    nameLabel: "Name",
    optional: "(optional)",
    passwordHint: "(8+ characters)",
    createCta: "Create account",
    creating: "Creating…",
    haveAccount: "Already a member?",
    genericError: "Could not create the account.",
    networkError: "Network error. Please try again.",
  },

  listings: {
    title: "Residences",
    countOne: "luxury listing",
    countOther: "luxury listings",
    countryThailand: "Thailand",
    countryDubai: "Dubai",
    countryMiami: "Miami",
    countryBali: "Bali",
    comingSoon: "Coming soon",
    searchPlaceholder: "Search area, project, keyword…",
    allRegions: "All regions",
    allTypes: "All types",
    allOfferTypes: "Buy or rent",
    samuiRegion: "Koh Samui (Surat Thani)",
    budgets: [
      "Any budget",
      "Up to €700K",
      "€700K – €1.3M",
      "€1.3M – €2.7M",
      "€2.7M +",
    ],
    sorts: ["Most recent", "Price — high to low", "Price — low to high", "Largest"],
    noResults: "No residences match those filters yet.",
    clearFilters: "Clear filters",
    prevPage: "Previous page",
    nextPage: "Next page",
    bed: "bed",
    bath: "bath",
    sqm: "sqm",
    sqmLand: "sqm land",
    forSale: "For sale",
    forRent: "For rent",
    signature: "Signature",
    priceOnApplication: "Price on application",
    perMonth: "/ month",
    annualRent: "Annual rent (paid yearly, as is standard in Dubai)",
  },

  detail: {
    back: "Back to residences",
    description: "Description",
    features: "Features",
    location: "Location",
    type: "Type",
    bedrooms: "Bedrooms",
    bathrooms: "Bathrooms",
    interiorArea: "Interior area",
    landArea: "Land area",
    region: "Region",
    furnished: "Furnished",
    yes: "Yes",
    listingAgency: "Listing agency",
    viewOnAgency: "View on agency website",
    agencyDisclaimer:
      "You are introduced directly to the agency that holds this listing. Palais Horizon is not the seller.",
    moreIn: "More in",
  },

  account: {
    title: "My account",
    name: "Name",
    email: "Email",
    membership: "Membership",
    plan: "Plan",
    renewsEnds: "Renews / ends",
    ended: "Ended",
    browse: "Browse residences",
    statuses: {
      none: "No membership",
      active: "Active",
      trialing: "Trial",
      past_due: "Payment past due",
      canceled: "Canceled",
    },
    manageBilling: "Manage billing / cancel",
    opening: "Opening…",
    startMembership: "Start a membership",
    reactivate: "Reactivate membership",
    viewMembership: "View membership",
    signOut: "Sign out",
    portalError: "Could not open the billing portal.",
  },

  success: {
    title: "Welcome to Palais Horizon",
    body: "Your membership is being activated. This can take a few seconds while the payment confirms.",
    cta: "Enter the collection",
    hint: "If the catalogue still asks for a plan, refresh in a moment — the confirmation is finishing up.",
    claimTitle: "Your membership is active",
    claimBody: "Set a password so you can sign back in anytime — no account was needed to pay.",
    claimPasswordHint: "(8+ characters)",
    claimCta: "Set password & enter",
    claimSubmitting: "Setting up your account…",
    claimError: "Could not set your password. Please try again.",
    claimNetworkError: "Network error. Please try again.",
    existingTitle: "Your membership is active",
    existingBody: "This email already has a Palais Horizon account — sign in to access the collection.",
    existingCta: "Sign in",
    genericTitle: "Payment received",
    genericBody: "Sign in (or create an account with the same email) to access your membership.",
  },

  about: {
    title: "About Palais Horizon",
    p1: "Palais Horizon is an independent index of luxury property across Thailand. We gather listings from the country's leading agencies and portals, normalise them into one collection, and keep the link back to the original source so members deal with the mandated agency directly.",
    p2: "We do not hold property mandates and we are not a party to any sale or lease. Membership pays for curation and access — a single, searchable view of the top of the market and a direct line to every agency.",
    membersTitle: "What members get",
    members: [
      "Full asking prices in EUR and USD, with the exact location and map.",
      "Complete photo galleries and property detail.",
      "A one-click introduction to the listing agency's own page.",
      "Unlimited search and filtering across every region we cover.",
    ],
    sourcesTitle: "Where listings come from",
    sourcesBody:
      "Sources currently include Conrad Properties, Thailand-Property, Three Seasons Properties and other established luxury agencies. The collection is refreshed by an ingestion pipeline that filters out everything below the fine-property line.",
    cta: "View membership",
  },

  notFound: {
    title: "This address isn't in the book",
    body: "The page you were looking for has moved or never existed.",
    cta: "Back to Palais Horizon",
  },

  footer: {
    explore: "Explore",
    account: "Account",
    legal: "Legal",
    residences: "Residences",
    membership: "Membership",
    about: "About",
    signIn: "Sign in",
    createAccount: "Create account",
    myAccount: "My account",
    howItWorks: "How the index works",
    terms: "Terms of Service",
    privacy: "Privacy Policy",
    disclaimer:
      "An independent property index. Listings belong to their respective agencies; Palais Horizon links to the original source and is not the seller.",
  },

  languageNames: {
    en: "English",
    fr: "Français",
    de: "Deutsch",
  },
};

export type Dict = typeof en;
